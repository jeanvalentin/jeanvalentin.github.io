...
Software management on Linux is simple; software management on Windows is a never-ending pilgrimage. All major Linux distributions got that right, but Windows did not? Why? Microsoft could not come up with a package manager of their own<sup>1</sup>, so someone else did and provided forsaken Windows users with Chocolatey.

And if you think it does the same things apt does, but on Windows, then in most cases, you're right.
...
The thing is, at the time of this publication, Chocolatey's repositories are far more furnished than Winget's, which means if you are looking for an obscure application, Chocolatey will have it, but Winget will not.
